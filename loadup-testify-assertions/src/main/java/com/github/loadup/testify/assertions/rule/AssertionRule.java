...
public class AssertionRule {
  ...
  @Builder.Default private CompareOperator operator = CompareOperator.EQUALS;

  /**
   * The expected value for comparison. Can be a literal value, a variable reference (${=variable}),
   * or a Datafaker expression.
   */
  ...
  public static AssertionRule notNull(String field) {
    return AssertionRule.builder().field(field).operator(CompareOperator.NOT_NULL).build();
  }

  /** Create an ignore-order collection comparison rule. */
  public static AssertionRule ignoreOrder(String field) {
    return AssertionRule.builder().field(field).operator(CompareOperator.IGNORE_ORDER).build();
  }
}
