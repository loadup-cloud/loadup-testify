...
public class AssertionService {
  ...
  public void assertMap(
      Map<String, Object> actual, Map<String, Object> expected, Set<String> ignoreFields) {
  ...
  public void assertList(List<?> actual, List<?> expected, boolean ignoreOrder) {
    ...
    if (actual.size() != expected.size()) {
      throw new AssertionException(
          String.format(
              "List size mismatch: expected %d but was %d", expected.size(), actual.size()));
    }
  ...
  /**
   * Assert that database rows match expected data. Uses DataComparator for detailed field
   * comparison with support for: - JSON field comparison - DateTime tolerance - Variable resolution
   *
   ...
   * @throws AssertionException if any assertion fails
   */
  public void assertDatabaseRows(
  ...
  public void assertDatabaseRows(
    ...
    if (actualRows.size() != expectedRows.size()) {
      throw new AssertionException(
          String.format(
              "Row count mismatch: expected %d but was %d",
  ...
  /**
   * Assert that actual map contains expected values, only checking fields in expected. Extra fields
   * in actual are allowed (useful when actual has more fields than expected).
   */
  @SuppressWarnings("unchecked")
  private void assertMapWithExpectedFields(
      Map<String, Object> actual, Map<String, Object> expected, Set<String> ignoreFields) {
    for (Map.Entry<String, Object> entry : expected.entrySet()) {
      String key = entry.getKey();
      if (ignoreFields != null && ignoreFields.contains(key)) {
      ...
      if (expectedValue instanceof Map) {
        ...
        assertMapWithExpectedFields(
            actualNestedMap, (Map<String, Object>) expectedValue, nestedIgnoreFields);
      } else {
        AssertionRule rule =
            AssertionRule.builder()
                .field(key)
  ...
  /**
   * Extract nested ignore fields for a given key prefix. E.g., for key "role" and ignoreFields
   * ["role.id", "role.description", "id"], returns ["id", "description"].
   */
  ...
  private void assertEqualsInternal(Object actual, Object expected, String context) {
    ...
    if (!Objects.equals(String.valueOf(actual), String.valueOf(expected))) {
      throw new AssertionException(
          String.format("%s mismatch: expected '%s' but was '%s'", context, expected, actual));
    }
  }
}
