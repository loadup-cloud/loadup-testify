 ...
 *
 * <p>Supported patterns: - ${faker.name.firstName} - Generate random data using Datafaker -
 * ${=variable_name} - Reference a variable from the shared pool - ${variable_name} - Simple
 * variable substitution from the shared pool
 */
@Slf4j
@Component
public class VariableResolver {
   ...
   * @param captureVariableMap optional map to track captured variables (key: original expression,
   *     value: generated value)
   * @return the resolved string
   */
  public String resolve(
      String input, boolean captureToPool, Map<String, Object> captureVariableMap) {
    if (input == null) {
      return null;
    }
  ...
  private String resolveFakerExpressions(
      String input, boolean captureToPool, Map<String, Object> captureVariableMap) {
  ...
  private String generateFakerValue(String expression) {
    try {
      ...
      for (String part : parts) {
        ...
        Method method =
            METHOD_CACHE.computeIfAbsent(cacheKey, key -> findMethod(currentClass, part));

        if (method == null) {
          throw new VariableResolutionException(
              "Unknown Datafaker method: " + part + " in expression: " + expression);
        }
      ...
      throw new VariableResolutionException(
          "Failed to generate Datafaker value for expression: " + expression, e);
    }
  }
