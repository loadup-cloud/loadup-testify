 ...
 *
 * <p>Supports multiple CSV files per ExpectedData directory, each corresponding to a different
 * table.
 */
@Slf4j
@Service
public class ExpectedDataService {
   ...
   */
  public Map<String, List<Map<String, String>>> loadExpectedData(
      Class<?> testClass, String serviceName, String methodName, String caseId) {
    Path expectedDataDir =
        PathUtils.getExpectedDataDirectory(testClass, serviceName, methodName, caseId);
    Map<String, List<Map<String, String>>> expectedData = new LinkedHashMap<>();

    if (!PathUtils.exists(expectedDataDir)) {
      log.debug(
          "No ExpectedData directory found for case: {}.{}/{}", serviceName, methodName, caseId);
      return expectedData;
    }
    ...
    if (csvFiles.length == 0) {
      log.debug(
          "No CSV files found in ExpectedData directory for case: {}.{}/{}",
          serviceName,
          methodName,
          caseId);
      return expectedData;
    ...
    log.info(
        "Found {} CSV files in ExpectedData for {}.{}/{}",
        csvFiles.length,
        serviceName,
        methodName,
        caseId);
  ...
  /**
   * Query actual data from the database based on expected data conditions. The first column in each
   * expected data row is used as the query condition.
   *
   * @param tableName the table name
   * @param expectedRows the expected rows (first column used as condition)
   * @return the actual data from the database
   */
  public List<Map<String, Object>> queryActualData(
      String tableName, List<Map<String, String>> expectedRows) {
    ...
    for (Map<String, String> expectedRow : expectedRows) {
      ...
      String sql =
          String.format(
              ...
              sanitizeIdentifier(tableName), sanitizeIdentifier(conditionColumn));
  ...
  public List<Map<String, Object>> queryData(
      String tableName, String conditionColumn, Object conditionValue) {
    String sql =
        String.format(
            "SELECT * FROM %s WHERE %s = ?",
            sanitizeIdentifier(tableName), sanitizeIdentifier(conditionColumn));
