...
/**
 * Service for preparing test data by inserting data into the database from CSV
 * files.
 *
 * <p>
 * Supports multiple CSV files per PrepareData directory, each corresponding to
 * a different
 * table.
 */
@Slf4j
...
public class PrepareDataService {
  ...
  /**
   * Prepare test data by loading CSV files from the PrepareData directory and
   * inserting into the
   * database. This method first clears the target tables, then inserts the data.
   *
  ...
  public void prepareData(
    ...
    if (!PathUtils.exists(prepareDataDir)) {
      log.debug(
          "No PrepareData directory found for case: {}.{}/{}", serviceName, methodName, caseId);
      return;
    }

    // 1. Execute SQL scripts first (e.g., for schema changes or complex setup)
    Path[] sqlFiles = PathUtils.getSqlFiles(prepareDataDir);
    ...
    if (csvFiles.length == 0 && sqlFiles.length == 0) {
      log.debug(
          "No CSV or SQL files found in PrepareData directory for case: {}.{}/{}",
          serviceName,
          methodName,
          caseId);
      return;
    ...
    if (csvFiles.length > 0) {
      log.info(
          "Found {} CSV files in PrepareData for {}.{}/{}",
          csvFiles.length,
          serviceName,
          methodName,
          caseId);
   ...
   */
  @Transactional
  public void executeSqlScript(Path sqlPath) {
    try {
      java.sql.Connection connection = java.util.Objects.requireNonNull(jdbcTemplate.getDataSource()).getConnection();
      try {
        log.info("Executing SQL script: {}", sqlPath.getFileName());
        org.springframework.jdbc.datasource.init.ScriptUtils.executeSqlScript(
            connection, new org.springframework.core.io.FileSystemResource(sqlPath.toFile()));
      } finally {
      ...
      throw new DataLoadingException("Failed to execute SQL script: " + sqlPath, e);
    }
  }
}
