...
public class MockEngine {
    ...
    public MockEngine(
            ApplicationContext applicationContext, VariableEngine variableEngine, MockInterceptor mockInterceptor) {
    ...
    private void applySingleMock(MockConfig config, Map<String, Object> context) throws Exception {
        ...
        if (config.thenThrow() != null) {
            ...
            // 1. 解析异常消息中的变量
            String resolvedMsg = variableEngine
                    .evaluate(throwExMessage, VariableContext.get())
                    .toString();

            // 2. 加载异常类
            Class<? extends Throwable> exceptionClass = (Class<? extends Throwable>) Class.forName(throwExClass);
    ...
    private Class<?> getMethodReturnType(Object bean, MockConfig config) {
        ...
        if (methods.length > 1) {
            long distinctTypes =
                    Arrays.stream(methods).map(Method::getReturnType).distinct().count();
            if (distinctTypes > 1) {
                log.warn(
