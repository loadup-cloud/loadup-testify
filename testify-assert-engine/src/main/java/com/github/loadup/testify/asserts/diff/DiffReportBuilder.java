...
public class DiffReportBuilder {
  ...
  public static String build(String tableName, List<RowDiff> diffs) {
    ...
    sb.append(LINE_SEP)
        .append("❌ [Database Assertion Failed] Table: ")
        .append(tableName)
        .append(LINE_SEP);
    ...
    for (RowDiff row : diffs) {
      sb.append(
              String.format(
                  "Row Index: [%d] | Status: %-8s | Match Criteria: %s",
                  row.index(), row.status(), formatMatchCriteria(row.rawExpected())))
          .append(LINE_SEP);

      if ("MISSING".equals(row.status())) {
        sb.append(
                "  ↳ Error: In the database, no record was found that matches the above criteria.")
            .append(LINE_SEP);
  ...
  private static String drawHeader() {
    return String.format(
        ...
        + LINE_SEP
        + "  | "
        + "-".repeat(COL_WIDTH_FIELD)
        + " | "
        + "-".repeat(COL_WIDTH_VAL)
        + " | "
        + "-".repeat(COL_WIDTH_VAL)
        + " | "
        + "-".repeat(15)
