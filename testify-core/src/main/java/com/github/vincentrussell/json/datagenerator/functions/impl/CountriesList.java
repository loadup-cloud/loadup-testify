...
public class CountriesList {
    ...
    private static final Map<String, String> COUNTRIES_LIST_JSON = JsonUtil.toMap(COUNTRIES_LIST);
    ...
    public String getCountriesList(final String... countryCodes) {
        if (countryCodes != null && countryCodes.length != 0) {
            if (countryCodes.length == 1) {
                return COUNTRIES_LIST_JSON.get(countryCodes[0]);
            }

            Map<String, String> jsonObjectToReturn = new HashMap<>();
            for (String countryCode : countryCodes) {
                if (COUNTRIES_LIST_JSON.containsKey(countryCode)) {
                    jsonObjectToReturn.put(countryCode, COUNTRIES_LIST_JSON.get(countryCode));
                }
            }
