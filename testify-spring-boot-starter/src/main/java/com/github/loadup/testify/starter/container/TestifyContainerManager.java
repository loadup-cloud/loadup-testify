 ...
 * Uses reflection to avoid compile-time dependencies on Testcontainers.
 */
public class TestifyContainerManager {
    ...
    private void initializeContainers() {
        try {
            ...
            Object dockerImage = dockerImageClass.getMethod("parse", String.class)
                    .invoke(null, "mysql:8.0");

            mySQLContainer = mysqlContainerClass.getConstructor(dockerImageClass)
                    .newInstance(dockerImage);

            // Configure MySQL container
            mySQLContainer.getClass().getMethod("withDatabaseName", String.class)
                    .invoke(mySQLContainer, "testdb");
            mySQLContainer.getClass().getMethod("withUsername", String.class)
                    .invoke(mySQLContainer, "test");
            mySQLContainer.getClass().getMethod("withPassword", String.class)
                    .invoke(mySQLContainer, "test");

            // Start container
            mySQLContainer.getClass().getMethod("start").invoke(mySQLContainer);
            String jdbcUrl = (String) mySQLContainer.getClass().getMethod("getJdbcUrl").invoke(mySQLContainer);
            ...
            Object redisImage = dockerImageClass.getMethod("parse", String.class)
                    .invoke(null, "redis:7-alpine");

            redisContainer = genericContainerClass.getConstructor(dockerImageClass)
                    .newInstance(redisImage);

            // Configure Redis container
            redisContainer.getClass().getMethod("withExposedPorts", int[].class)
                    .invoke(redisContainer, new int[]{6379});

            // Start Redis container
            redisContainer.getClass().getMethod("start").invoke(redisContainer);
            String redisHost = (String) redisContainer.getClass().getMethod("getHost").invoke(redisContainer);
            Integer redisPort = (Integer) redisContainer.getClass().getMethod("getMappedPort", int.class)
    ...
    public Integer getRedisPort() {
        if (redisContainer != null) {
            try {
                return (Integer) redisContainer.getClass().getMethod("getMappedPort", int.class)
                        .invoke(redisContainer, 6379);
            } catch (Exception e) {
